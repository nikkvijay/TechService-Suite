...
public class PrinterInfo implements ActionListener {
	...
	PrinterInfo(){
		...
		f1=new JFrame();
		f1.setSize(700,850);
		f1.setTitle("PrinterInfo");
		f1.setLayout(null);
		
		l1=new JLabel("PRINTER");
		...
		l5=new JLabel("Color Output:");
		l5.setBounds(92,300,200,45);
	...
	public void actionPerformed(ActionEvent e) {
	    ...
	    if (source == b1) {
	        ...
	        if (tf1.getText().trim().isEmpty() ||
	                ...
	                tf4.getText().trim().isEmpty() ||
	                tf5.getText().trim().isEmpty() ||
	                tf6.getText().trim().isEmpty() ||
	                tf7.getText().trim().isEmpty() ||
	                tf8.getText().trim().isEmpty() ||  
	                ta.getText().trim().isEmpty()) {
        ...
        String dpi = tf3.getText();
        String colorop = tf4.getText();
        String cust = tf5.getText();
        String mobno = tf6.getText();
        String email = tf7.getText() + (String) cb1.getSelectedItem();
        String issue = ta.getText();
        String charges = tf8.getText();
        String attachmentPath = "";
        
        InsertIntoDatabase.printerdata(comp, model, dpi, colorop, cust, mobno, email, issue, charges);
        ...
        if (userSelection == JFileChooser.APPROVE_OPTION) {
            ...
            DynamicBilling.generatePDF(comp, model, dpi, colorop, cust, mobno, email, issue, charges, attachmentPath);
            JOptionPane.showMessageDialog(f1, "PDF saved: " + attachmentPath);

            
            try {
                EmailSender.sendEmail(email, "Invoice and Details", "Here are your Printer details and invoice.", attachmentPath);
